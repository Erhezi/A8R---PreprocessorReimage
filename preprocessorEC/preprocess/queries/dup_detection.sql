...
-- name: ccx_match_distributor_premier_mfg_set
-- Set-based DISTRIBUTOR PREMIER match on reduced mfg numbers.
SELECT
    ccx.CCX_pkid,
    ccx.OrganizationEID,
    ccx.Organization,
...
-- name: ccx_match_distributor_premier_vendor_set
-- Set-based DISTRIBUTOR PREMIER match on reduced vendor numbers.
SELECT
    ccx.CCX_pkid,
    ccx.OrganizationEID,
    ccx.Organization,
...
-- name: ccx_match_distributor_local_mfg_set
-- Set-based DISTRIBUTOR LOCAL match on CCX reduced mfg numbers.
SELECT
    ccx.CCX_pkid,
    ccx.OrganizationEID,
    ccx.Organization,
...
-- name: ccx_match_distributor_local_vendor_set
-- Set-based DISTRIBUTOR LOCAL cross-match on CCX reduced vendor numbers.
SELECT
    ccx.CCX_pkid,
    ccx.OrganizationEID,
    ccx.Organization,
...
LEFT JOIN [Preprocessor].[CCXInforSyncedContractHeader] hdr
   ...
   AND hdr.ContractID    = ccx.ContractID
   AND hdr.ERPVendorID   = ccx.ERPVendorID
WHERE ccx.reduced_vendor_num_ccx IN :reduced_mfg_nums
  AND (
      :org_eid = '105188574'
      OR ccx.OrganizationEID IN (:org_eid, '105188574')
  );
